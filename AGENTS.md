...
User-scope guidance for AI coding agents like Codex CLI (`~/.codex/AGENTS.md`).
No repo-specific rules here.
...
- Exploration: if the file or symbol is clear, grep/read directly; enter broad
  exploration deliberately, mindful of its cost.
- request_user_input when requirements or options are ambiguous;
  simple confirmations in plain text.
- Checkpoint (double-check) only for big changes, when stuck, or right before
