...
> A portable skill for Claude Code, Codex, Antigravity — or any AI agent — that turns long-form Markdown (plans, specs, system designs, RFCs, runbooks, postmortems, brainstorms, notes) into a single themed HTML file with sidebar TOC, inline SVG diagrams, KaTeX math, step timelines, callouts, comparison cards, and a light/dark Claude-orange theme.
...
[![Languages](https://img.shields.io/badge/lang-EN%20%C2%B7%20KO-blue.svg)](#languages)
...
You type `/md2html anydoc.md`. Your agent reads the file, picks components from `components.md`, and assembles one polished HTML file via `scripts/build.py` (python3 stdlib only — preinstalled on macOS/Linux). No build step to configure, no server, no Node, no pip install.
...
It's not a Markdown-to-HTML converter. **It's an analyzer.** The agent decides which sections become SVG diagrams, which become numbered step cards, which become pros-cons tables, which become collapsible "deep dive" panels. The result feels designed, not converted.
...
- **Diagrams instead of paragraphs.** Three-hop flows become theme-native HTML/SVG flow diagrams (KaTeX-capable labels, print-safe); complex diagrams (dense flowcharts, sequence, ER, state, architecture) become hand-drawn inline SVG in the same theme tokens. Trade-off discussions become side-by-side comparison cards. The agent decides — you don't write any of it.
...
- **One file. Email it. Drop it in Slack.** The HTML embeds its CSS and theme JS. The template already declares KaTeX and Google Fonts CDNs; when offline, the page still opens and degrades gracefully.
...
| "A calls B, B writes to DB" prose               | Native HTML/SVG flow (inline SVG diagram if complex) |
...
UI labels follow the source: **Korean** sources get Korean labels (목차, ~N분 소요, ★ 추천, …), everything else gets **English**. The agent sets `<html lang="...">` plus the `--rec-label` CSS variable accordingly. Body content always stays in the source language.

## Demo

See [`examples/example-plan.md`](examples/example-plan.md) → [`examples/example-plan.html`](examples/example-plan.html) (rendered). The screenshots at the top of this README are from a longer test spec — a game economy design doc.
...
├── template.html    # HTML skeleton: CSS + KaTeX + theme JS
...
- [ ] Dedicated gantt/roadmap component
