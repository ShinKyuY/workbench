...
- **결함 징후 탐지** — 크기(Long Method/Large Class), 구조(God Class,
  Feature Envy), 중복, 깊은 중첩, 결합도(Shotgun Surgery 등), 네이밍.
...
**확인 포인트 ①**: Phase 1+2 결과를 함께 사용자에게 보고하고,
범위와 우선순위를 합의한 뒤 Phase 3으로 넘어간다.
...
**Execute subagent** (`agents/execute.md`)를 spawn한다.
