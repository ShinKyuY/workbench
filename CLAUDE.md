...
- **서브에이전트**: 파일/심볼이 명확하면 직접 grep, 광범위 탐색은 Explore.
- **Agent teams**: 사용자가 "팀/team/agent teams"를 명시하면 반드시
  `TeamCreate` → `TaskCreate` → `Agent(team_name=..., name=...)` 순서로
  정식 팀을 구성한다. 명명된 subagent 단독 호출로 대체 금지(Task 공유·
  상호 통신·shutdown 라이프사이클이 다름). teammate는 역할이 드러나는
  이름(`code-explorer` 등) + `mode: plan` + 생성 프롬프트에 작업 컨텍스트를
  충분히 담는다(리더 대화 기록 비상속). 단일 위임이면 `Agent` 단독으로 충분.
- **AskUserQuestion**: 옵션 선택이 모호하거나 요구사항이 불명확할 때
  쟁점을 정리해 객관식으로 묻는다. 단순 확인은 그냥 텍스트로.
- **스킬**: `superpowers`는 중요한 작업에서 트리거 조건에 명확히 맞을 때만
  호출한다. 사소한 작업에는 생략한다.
